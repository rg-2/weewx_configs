...
class LaCrosseTH3Packet(Packet):
    IDENTIFIER = "LaCrosse-TH3"
    @staticmethod
    def parse_json(obj):
        pkt = dict()
        pkt['dateTime'] = Packet.parse_time(obj.get('time'))
        ...
        pkt['temperature'] = Packet.get_float(obj, 'temperature_C')
        pkt['humidity'] = Packet.get_float(obj, 'humidity')
        pkt = Packet.add_identifiers(pkt, sensor_id, LaCrosseTH3Packet.__name__)
        return pkt

class LaCrosseW1Packet(Packet):
    IDENTIFIER = "LaCrosse-W1"
    @staticmethod
    def parse_json(obj):
        pkt = dict()
        pkt['dateTime'] = Packet.parse_time(obj.get('time'))
        ...
        sensor_id = obj.get('id')
        pkt['wind_speed'] = Packet.get_float(obj, 'wind_avg_km_h')
        pkt = Packet.add_identifiers(pkt, sensor_id, LaCrosseW1Packet.__name__)
        return pkt
